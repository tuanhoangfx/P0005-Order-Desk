-- P0005 Order Desk — sheet_sources + upsert keys for CzP Seller import
-- Data Box plane bklxcjrkhrevdcqjscku
...
-- Upsert keys for sheet import
do $$
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'order_desk_customers_user_external_buyer_unique'
  ) then
    alter table public.order_desk_customers
      add constraint order_desk_customers_user_external_buyer_unique unique (user_id, external_buyer_id);
  end if;
end $$;
...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'order_desk_orders_user_external_order_unique'
  ) then
    alter table public.order_desk_orders
      add constraint order_desk_orders_user_external_order_unique unique (user_id, external_order_id);
  end if;
end $$;
